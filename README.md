...
项目包含一个基于浏览器的可视化界面（`web/index.html`），无需命令行即可完成聚类、预测、浏览等核心操作。

### 界面功能

| 功能 | 说明 |
|------|------|
| **一键更新** | 在网页中一键执行任务级聚类 + 负荷映射 + 监督模型训练，等价于依次运行核心脚本 |
| **单 Session 预测** | 输入 session 目录路径，预测其中每个任务的 cluster 与相对认知负荷等级 |
| **任务级浏览** | 查看离线聚类结果，支持按 session 筛选、排序、分页、导出 CSV |
| **实时监控** | 持续监控指定目录，自动预测新数据的认知负荷，图表实时可视化 |
...
- **顶部导航**：切换「一键更新」「单 Session 预测」「任务级浏览」「实时监控」四个功能区
- **单 Session 预测**：输入 session 目录（如 `data/20260124_140152`），返回每个 task 的 cluster、相对负荷等级、2D 坐标
- **任务级浏览**：分页表格 + 负荷分布柱状图，支持导出为 CSV
- **实时监控**：启动后自动轮询，柱状图展示最近 40 条预测的认知负荷等级变化
