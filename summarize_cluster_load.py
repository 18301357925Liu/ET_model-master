...
def main() -> int:
    ...
    if args.mapping_mode == "manual":
        ...
        if not score_cols_weights:
            ...
            n = float(len(summary))
            if n <= 1:
                # 只有一个 cluster 时，直接给一个中间等级，避免除零/NaN
                levels = pd.Series([2], index=summary.index, dtype="float64")
            else:
                # 线性映射到 [1,4]，再四舍五入，确保覆盖面稳定
                frac = (order - 1.0) / (n - 1.0)
                levels = 1.0 + 3.0 * frac
            # 对异常值做稳健处理，避免 NaN/inf 导致 astype 抛错
            levels = levels.clip(1, 4)
            levels = levels.fillna(2.0).replace([np.inf, -np.inf], 2.0)
            levels = levels.round().astype("int64")
