...
# 兼容两种运行方式：
# - 作为包模块：python -m Model.ET_model.cluster_cognitive_data
# - 作为当前仓库根目录下的脚本：python cluster_cognitive_data.py
try:  # 优先尝试当前仓库的扁平布局（本仓库使用）
    from eyerunn_cluster import cluster_features  # type: ignore
    from eyerunn_cluster.cognitive import (  # type: ignore
        discover_sessions,
        extract_cognitive_features,
    )
except Exception:  # noqa: BLE001
    # 回退到相对导入，以兼容原始 EyeTrace 仓库中的包结构
    from .eyerunn_cluster import cluster_features  # type: ignore
    from .eyerunn_cluster.cognitive import (  # type: ignore
        discover_sessions,
        extract_cognitive_features,
